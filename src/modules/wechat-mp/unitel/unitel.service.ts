...
export class UnitelService {
  ...
  private readonly API_ENDPOINTS = {
    ...
    POSTPAID_BILL: '/service/unitel', // 获取后付费账单
    PAY_POSTPAID_BILL: '/service/payment', // 支付后付费账单
    CHECK_TRANSACTION: '/service/check_transaction_result', // 查询充值结算结果
  };
  ...
  async getServiceTypes(dto: QueryServiceDto): Promise<ServiceTypeResponse> {
    ...
    try {
      ...
      const data: ServiceTypeResponse = response.data;
  ...
  async recharge(dto: RechargeDto): Promise<RechargeResponse> {
    ...
    try {
      const firstTransaction = dto.transactions[0];
      this.logger.log({
        message: '发起话费充值请求',
        msisdn: dto.msisdn,
        card: dto.card,
        journal_id: firstTransaction?.journal_id ?? '',
      });
      ...
      const data: RechargeResponse = response.data;
  ...
  async activateDataPackage(dto: DataPackageDto): Promise<DataPackageResponse> {
    ...
    try {
      ...
      const data: DataPackageResponse = response.data;
  ...
  async getPostpaidBill(
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post<GetPostpaidBillResponse>(
          `${this.apiUrl}${this.API_ENDPOINTS.POSTPAID_BILL}`,
          dto,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      const data: GetPostpaidBillResponse = response.data;
      ...
      const errorMessage =
        err.response?.data?.msg || 'Failed to get postpaid bill';

      this.logger.error('Unitel 获取后付费账单失败', {
        message: err.message,
        stack: err.stack,
        status: err.response?.status,
        apiMessage: err.response?.data?.msg,
   ...
   * 支付后付费账单
   */
  async payPostpaidBill(
    dto: PayPostpaidBillDto,
  ): Promise<PayPostpaidBillResponse> {
    const token = await this.getAccessToken();
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post<PayPostpaidBillResponse>(
          `${this.apiUrl}${this.API_ENDPOINTS.PAY_POSTPAID_BILL}`,
          dto,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      const data: PayPostpaidBillResponse = response.data;
      ...
      const errorMessage =
        err.response?.data?.msg || 'Failed to pay postpaid bill';

      this.logger.error('Unitel 支付后付费账单失败', {
        message: err.message,
        stack: err.stack,
        status: err.response?.status,
        apiMessage: err.response?.data?.msg,
  ...
  async checkTransactionResult(
    ...
    try {
      this.logger.log({
        message: '发起查询充值结算结果请求',
        seq_id: dto.seq_id,
      });

      const response = await firstValueFrom(
        this.httpService.post<CheckTransactionResponse>(
          `${this.apiUrl}${this.API_ENDPOINTS.CHECK_TRANSACTION}`,
          dto,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      const data: CheckTransactionResponse = response.data;
      ...
      const errorMessage =
        err.response?.data?.msg || 'Failed to check transaction result';

      this.logger.error('Unitel 查询充值结算结果失败', {
        message: err.message,
        stack: err.stack,
        status: err.response?.status,
        apiMessage: err.response?.data?.msg,
